...
import { useBehavior, useTableWeightData, useTypeBehvios } from "./behavior";
import './index.scss'

import { IOSSubjectProps, selectMulMapType } from "./types";

const OSSubjectWeight: FC<IOSSubjectProps> = ({
  title,
  data
}) => {

  const { tData, fdata, visiable, setVisiable, handleEvent } = useBehavior(selectMulMapType);
  const { headData, bodyData } = useTableWeightData(data.data);
  const { label, seriesData } = useTypeBehvios(fdata.mapType, data.data, 'weight', fdata.numType)
  return (
...
export default OSSubjectWeight;
